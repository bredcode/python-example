...
- 리팩토링 되어야할 사항
  - Player가 현재 name만 받도록 되어있다. hp와 atk를 생성자에서 파라미터로 받도록 하고, 기본값 hp = 100, atk = 10을 넣어주자. 그리고 Player를 불러올 때 name, hp, atk를 생성자를 통해 주입해주자
...
- 몬스터를 사냥하면 골드를 얻을 수 있는 시스템을 추가한다.
- 몬스터에게 dropMoney 함수를 만들고 죽을 시, 10 ~ 100 사이 골드를 드랍하게 만든다.
- 플레이어에게 getMoney 함수를 만들고 몬스터를 사냥하면 몬스터가 드랍한 골드를 획득하게 만든다 (변수명 money 사용).
- displayInfo에서 플레이어의 골드도 나타나게 변경한다.

20. 리팩토링

- 저장 시 골드도 저장되게 변경한다.
- 만약 object.py에 money를 선언하지 않고 몬스터와 플레이어에 따로 money라는 변수를 두어 구현하였다면, 동일한 money라는 멤버변수를 쓸 수 있도록 변경해본다.

21. save 파일 보안 강화

- 무작위 암호를 입력하여 우연하게 암호가 맞으면 내 저장 파일이 노출되게 된다.
- 암호 입력에 제한을 둔다.
  - 암호가 틀리더라도 최대 3번의 암호 입력을 받을 수 있도록 한다.
  - 암호가 틀릴 때 마다 `"암호가 틀렸습니다. 앞으로 {n}번 기회가 남았습니다"` 처럼 횟수가 나타나게 한다.
  - 이때 3번 모두 연속해서 틀리면 `"3번 연속 암호를 틀려 파일을 삭제합니다"`를 출력 후 파일을 삭제시킨다.
  - 카운트를 세는 변수명은 `errorCount`로 한다.
  - 파일을 지우는 모듈은 검색을 통해 찾아내본다.

---

1. saveData.save를 const로 빼기
2. load에서 saveData.save 파일이 존재하는지 먼저 확인하는 방식으로 바꾸고, 불러오기 시 파일이 없으면 저장된 파일이 없습니다를 먼저 출력하게 하기
3. 파일이 길어지니 gameLauncher에서 Game이라는 클래스 빼기
