...
- select라는 변수에 `원하는 번호를 입력해주세요 : `라는 안내와 함께 input을 받고
- "1"이면 play "2"면 load "3"이면 exit 함수를 호출하도록 한다.
...
- 그 후 아래와 같은 로직의 순서를 구현한다.
  ...
  `원하는 번호를 입력해주세요 : ` 라는 안내와 함께 select 변수에 값을 받는다.
